...
public class StripeServices
{
        ...
        try
        {
            ...
            if (cliente != null && cliente.IdCarritoNavigation != null && cliente.IdCarritoNavigation.ProductosEnCarrito != null)
            {
                ...
                if (listaProductos.Count > 0)
                {
                    ...
                    Venta venta = await _compraService.RealizarCompra(IdEstablecimiento, phoneNumber);
                    var checkout = await CreateStripeCheckout(listaProductos, venta);
    ...
    private async Task<Stripe.Checkout.Session> CreateStripeCheckout(List<ProductosEnCarrito> productos, Venta venta)
    {
        ...
        venta.IdTransaccion = session.PaymentIntentId;
        // para guardar la asignacion del id
